...
void bluetoothEventHandler(uint32_t event, void *eventParameter)
{
    switch(event)
    {
        ...
        case CY_BLE_EVT_GAP_DEVICE_DISCONNECTED:
        {
            ...
            buzzer_flag = 1;
            Cy_GPIO_Write(ONR_LED_PORT, ONR_LED_NUM, 0u);
            break;
        }
        case CY_BLE_EVT_GATT_CONNECT_IND:
        {
            ...
            buzzer_flag = 1;
            Cy_GPIO_Write(ONR_LED_PORT, ONR_LED_NUM, 1u);
            break;
        }
        case CY_BLE_EVT_GATTS_WRITE_REQ:
        {
            ...
            if ( writeReqParam->handleValPair.attrHandle == CY_BLE_DEVICE_INTERFACE_DEVICE_OUTBOUND_CHAR_HANDLE ) {
                if (writeReqParam->handleValPair.value.len > 0) {
                    esp_sensor = writeReqParam->handleValPair.value.val[0];
                     //recieve_flag = 1;
                }
            }
...
int main(void)
{
    //Cy_GPIO_Write(BUZZER_PORT, BUZZER_NUM, 1u);
    ...
    */
    ...
    const float VOLT_THRESHOLDS[5] = {1.20f, 1.50f, 1.60f, 1.60f, 1.60f};
    const float VOLT_MIN[5] = {0.91f, 1.04f, 1.18f, 1.19f, 1.27f};
    const float VOLT_MAX[5] = {1.60f, 2.05f, 1.96f, 2.15f, 2.03f};
    
    float volts[5]; // Create an array to hold all 5 voltage readings
    float flex_value_float[5];
    uint8_t flex_value_int[5];
    ...
    {   /*
        if (buzzer_flag == 1) {
            Cy_GPIO_Write(BUZZER_PORT, BUZZER_NUM, 1u);
            CyDelay(1000);
            Cy_GPIO_Write(BUZZER_PORT, BUZZER_NUM, 0u);
            buzzer_flag = 0;
        }
        if (esp_sensor <= ULTRASONIC_THRESH) {
            // Cy_GPIO_Write(ONR_LED_PORT, ONR_LED_NUM, 1u);
            buzzer_flag = 1;
            printf("Distance Measurement: %d\n\n\r", esp_sensor);
        }
        else {
            //Cy_GPIO_Write(ONR_LED_PORT, ONR_LED_NUM, ON);
            printf("Distance Measurement: %d\n\n\r", esp_sensor);
        }*/
        ...
        if (esp_sensor <= ULTRASONIC_THRESH) {
            Cy_GPIO_Write(BUZZER_PORT, BUZZER_NUM, ON);  // keep ON while close
            Cy_GPIO_Write(RED_LED_PORT, RED_LED_NUM, ON);
           // printf("Distance Measurement: %d\n\n\r", esp_sensor);
        } else {
            Cy_GPIO_Write(BUZZER_PORT, BUZZER_NUM, OFF);  // OFF when clear
            Cy_GPIO_Write(RED_LED_PORT, RED_LED_NUM, OFF);
            //printf("Distance Measurement: %d\n\n\r", esp_sensor);
        }
       ...
       */
        ...
        for (uint32_t i = 0; i < 5; i++) 
        {
            ...
            flex_value_float[i] = (255*(volts[i] - VOLT_MIN[i])/(VOLT_MAX[i]-VOLT_MIN[i]));
            if (flex_value_float[i]<0)
                flex_value_float[i] = 0;
            else if (flex_value_float[i]>255)
                flex_value_float[i] = 255;
            ...
            // 2. Print it out
            printf("Volt_%s: %f\r\n", num_names[i], flex_value_float[i]);
        }
        ...
        switch (gesture_num)
        {
            ...
            case 0x01:
                //4 curled - REVERSE
                instant_velocity = -flex_value_float[0];
                break;
            case 0x03:
            case 0x02:
                //pointing - FORWARD
        ...
        float volts_out = instant_velocity;
        ...
        int16_t batt_val = Cy_SAR_GetResult16(SAR, 5);
        float batt_val_float = Cy_SAR_CountsTo_Volts(SAR, 5, batt_val);
     ...
     //      volts = 0.0;
       ...
       // }
        ...
        sendBLENotification(volts_out, angle);
        CyDelay(100);
        //printf("CHAR: %c\r\n", gestureChar);
